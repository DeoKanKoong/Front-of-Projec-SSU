...
public class PwVerificationController {

    private final EmailService emailService;

    @GetMapping("/pwVerification")
    public String getPwVerificationPage() {
        return "ForgottenPw"; // ForgottenPw.html을 반환
    }

    @PostMapping("/sendPwVerificationEmail")
    public String sendPwVerificationEmail(@RequestParam("email") String email, Model model) {
        try {
        ...
        return "ForgottenPw"; // 결과를 보여주는 페이지로 이동
    }

    @PostMapping("/verifyPwCode")
    public String verifyPwCode(@RequestParam("code") String code, Model model) {
        try {
