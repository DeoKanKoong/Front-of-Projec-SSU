document.addEventListener('DOMContentLoaded', function() {
    ...
    function validateUserId() {
        ...
        const regex = /^[A-Za-z]{1,8}[0-9]{0,4}$/;
        const isValid = regex.test(value);
        const errorField = document.getElementById('userIdError');

        if (!isValid) {
            errorField.textContent = '아이디는 영어 8글자 이하 + 숫자 4개 이하로 구성되어야 합니다.';
        } else {
    ...
    confirmButton.addEventListener('click', function() {
        ...
        userInfo.portfolios = Array.from(document.querySelectorAll('#portfolioSection .portfolio-link input')).map(input => input.value);
        ...
        const data = {
            ...
            portfolios: userInfo.portfolios
        };
...
function addPortfolio() {
    if (portfolioCount < 3) {
        ...
        newPortfolio.className = 'portfolio-link';
        newPortfolio.innerHTML = `
            <div class="portfolio-label">포트폴리오 링크</div>
            <input type="url" class="input-field">
            <div class="action-button" onclick="removePortfolio(this)"></div>
        `;
        portfolioSection.insertBefore(newPortfolio, portfolioSection.lastElementChild);
        portfolioCount++;

        if (portfolioCount === 3) {
            document.querySelector('.action-button.add').style.display = 'none';
        }
    }
}
...
function removePortfolio(button) {
    const portfolioSection = document.getElementById('portfolioSection');
    portfolioSection.removeChild(button.parentElement);
    portfolioCount--;
    document.querySelector('.action-button.add').style.display = 'flex';
}
